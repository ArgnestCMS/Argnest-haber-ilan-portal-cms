...
<section class="mx-auto max-w-6xl px-4 py-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <aside class="order-last w-full max-w-none space-y-3 lg:order-none lg:col-span-4 lg:max-w-[240px]">
            ...
            @if(($portal['categoryBlocks'] ?? collect())->isNotEmpty())
                <section class="grid gap-2 md:grid-cols-2">
                    @foreach($portal['categoryBlocks'] as $block)
                        <div class="theme-card w-full overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 md:max-w-[260px]">
            ...
            <section>
                ...
                <div class="grid gap-2 md:grid-cols-2">
                    @forelse($latestCompact as $item)
                        <a href="/ilan/{{ $item->slug }}" class="theme-card flex w-full gap-2 rounded-2xl bg-white p-2 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md md:max-w-[260px]">
                            <div class="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-slate-100">
                                @if($item->image)
                                    <img
                                        src="{{ asset('storage/' . (str_contains($item->image, '/') ? $item->image : 'announcements/' . $item->image)) }}"
